...
import { LogOut, Plus, PlayCircle, Clock, CheckCircle, Settings, Users, BarChart2 } from 'lucide-react';
...
const ESTADO_CFG = {
  ...
  borrador:   { label: 'Borrador',   bg: 'bg-yellow-100',text: 'text-yellow-700',dot: 'bg-yellow-400',Icon: Settings },
};
    ...
    <main className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <header className="w-full bg-brand shadow-md sticky top-0 z-10">
        <div className="max-w-2xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={() => { sessionStorage.removeItem('usuario'); router.push('/'); }}
      ...
      <div className="max-w-2xl mx-auto w-full px-4 py-6 flex flex-col gap-6">
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 bg-brand hover:bg-brand-hover active:bg-brand-active text-white font-bold py-4 rounded-xl transition-colors shadow-md"
        >
          <Plus size={20} />
          Crear nueva sesión
        </button>
        ...
        <div>
          ...
          <div className="flex flex-col gap-3">
            ...
            {filtradas.map((sesion) => {
              ...
              return (
                <div key={sesion.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <div className="flex-1 min-w-0">
                      <p className="font-bold text-gray-900 text-sm leading-snug truncate">{sesion.nombre}</p>
                      <p className="text-xs text-gray-400 font-mono mt-0.5">{sesion.id}</p>
                    </div>
                  ...
                  <div className="flex items-center justify-between mt-3">
                    <div className="flex items-center gap-3 text-xs text-gray-400">
                      <span className="flex items-center gap-1"><Users size={11} /> {sesion.preguntas?.length || 0} preguntas</span>
                      <span>{sesion.fecha}</span>
                    </div>
                    <button
                      onClick={() => router.push(`/admin/sesion/${sesion.id}`)}
